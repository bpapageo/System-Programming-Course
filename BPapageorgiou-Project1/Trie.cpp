...
void Trienode::Postlistinsert(int id){
		if(ppt==NULL){
			ppt=new Postlist();
			ppt->insert(id);
		}
...
Trienode* Trienode::insertatlevel(char letter,int id){
		...
		if(next==NULL){
			...
			while(current->next!=NULL){
				if((current->next->x)==letter){
					...
					if(id!=-1){
						current->next->Postlistinsert(id);
					}
...
void Trie::insert(char* w,int id){

	Trienode* current = root;
	unsigned int i=0;
	while(i<strlen(w)){
		if(i==strlen(w)-1){
			current=current->insertatlevel(w[i],id);
			break;	
		}
		current=current->insertatlevel(w[i],-1);	
		i++;	
	}

}

int Trie::termf(char* w,int id){
	Trienode* current = root;
	unsigned int i=0;
	while(i<strlen(w)){
		if(i==strlen(w)-1){
	...
	return current->ppt->postlisttermf(id);
